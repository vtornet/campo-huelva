...
import { signInWithPopup, signInWithRedirect, getRedirectResult, GoogleAuthProvider, signInWithEmailAndPassword, createUserWithEmailAndPassword, User } from "firebase/auth";
import { auth, isMobile, createGoogleProvider } from "@/lib/firebase";
...
export default function LoginPage() {
  ...
  // Verificar resultado de redirect al cargar (solo relevante para móviles)
  useEffect(() => {
    ...
    const checkRedirectResult = async () => {
      try {
        ...
        if (result) {
          console.log("[LOGIN] Redirect result exitoso:", result.user?.email);
          // El AuthContext detectará el cambio de usuario
          // Esperar un momento y redirigir
          setTimeout(() => {
            router.push("/");
          }, 500);
        } else {
          console.log("[LOGIN] No hay redirect result pendiente");
        }
      } catch (err: any) {
        // No hay redirect pendiente, es normal
        if (err.code === 'auth/credential-already-in-use') {
          console.log("[LOGIN] Credencial ya en uso");
        } else {
          console.log("[LOGIN] Sin redirect pendiente:", err.code || err.message);
        }
      }
    };
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      console.log("[LOGIN] Iniciando Google Auth, móvil:", mobile);

      if (mobile) {
        // En móvil: usar REDIRECT (más fiable cuando hay app nativa de Google)
        console.log("[LOGIN] Usando signInWithRedirect para móvil");
        await signInWithRedirect(auth, provider);
        // La redirección ocurrirá automáticamente a la app
        // Al volver, el useEffect con getRedirectResult manejará el resultado
      } else {
        // En desktop: usar POPUP (mejor UX)
        console.log("[LOGIN] Usando signInWithPopup para desktop");
        await signInWithPopup(auth, provider);
        console.log("[LOGIN] Popup exitoso, redirigiendo...");
        router.push("/");
      }
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
      <div className="max-w-md w-full bg-white rounded-xl shadow-lg p-8">
        ...
        {error && (
          <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg shadow-lg text-sm max-w-md w-full mx-4 animate-slide-down">
        ...
        <div className="mt-6">
          ...
          <button
            ...
            className="mt-4 w-full flex justify-center items-center gap-2 bg-white border border-gray-300 text-gray-700 py-2 px-4 rounded-md hover:bg-gray-50 transition disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isProcessing ? (
              <>
