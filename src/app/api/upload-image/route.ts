...
export async function POST(request: Request) {
  try {
    ...
    console.log("[UploadImage] File:", file?.name, "Size:", file?.size, "Folder:", folder);
    ...
    console.log("[UploadImage] Base64 creado, tamaño:", base64.length);

    // Si hay API key de ImgBB, usar ImgBB
    if (IMGBB_API_KEY) {
      console.log("[UploadImage] Usando ImgBB");
      ...
      console.log("[UploadImage] ImgBB response:", imgbbData.success);
    ...
    console.log("[UploadImage] Usando Base64 directo (fallback)");
