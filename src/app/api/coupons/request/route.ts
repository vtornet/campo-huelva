...
export async function POST(request: Request) {
  try {
    ...
    // Crear cupón pendiente (marcado con el companyId en notes para identificación)
    const coupon = await prisma.coupon.create({
      data: {
        ...
        notes: `REQUEST:${user.companyProfile.id}|${reason.substring(0, 100)}|${companySize || 'N/A'}`,
      },
    });
