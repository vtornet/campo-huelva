            ...
            if (data && !data.error && data.exists !== false) {
              ...
              if (!isProfileComplete) {
                ...
                if (data.role === 'COMPANY') {
                  router.push(`/${locale}/profile/company`);
                } else if (data.role === 'FOREMAN') {
                  router.push(`/${locale}/profile/foreman`);
                } else if (data.role === 'ENGINEER') {
                  router.push(`/${locale}/profile/engineer`);
                } else if (data.role === 'ENCARGADO') {
                  router.push(`/${locale}/profile/encargado`);
                } else if (data.role === 'TRACTORISTA') {
                  router.push(`/${locale}/profile/tractorista`);
                } else {
        ...
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            {(userData?.role === 'ADMIN' || user?.email === "vtornet@gmail.com") && (
              <button onClick={() => router.push(`/${locale}/admin`)} className="text-xs bg-red-500 hover:bg-red-600 text-white py-2 px-3 rounded-xl transition-all duration-200 font-medium shadow-sm shadow-red-500/20">
                Admin
              </button>
            )}
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <LanguageSelector />
            <button onClick={() => router.push(`/${locale}/notifications`)} className="relative bg-slate-100 hover:bg-slate-200 text-slate-700 p-2 rounded-xl transition-all duration-200">
            ...
            <button onClick={() => router.push(`/${locale}/messages`)} className="relative bg-slate-100 hover:bg-slate-200 text-slate-700 p-2 rounded-xl transition-all duration-200">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {role === 'COMPANY' ? (
              ...
              <button
                onClick={() => router.push(`/${locale}/my-applications`)}
            ...
            <button onClick={() => router.push(`/${locale}/profile`)} className="flex items-center gap-2 bg-emerald-100 hover:bg-emerald-200 text-emerald-700 py-2 px-3 rounded-xl transition-all duration-200 text-sm font-medium">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 mb-6 flex flex-wrap gap-4 items-center shadow-black/5">
           ...
           {role === 'COMPANY' ? (
             <>
               ...
               <button onClick={() => router.push(`/${locale}/search`)} className="bg-gradient-to-r from-blue-600 to-blue-500 text-white py-3 px-5 rounded-xl font-semibold hover:from-blue-700 hover:to-blue-600 transition-all duration-200 shadow-sm shadow-blue-500/25 flex items-center gap-2">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
